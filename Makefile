...
run-dev: ## Run the agent in development mode with auto-reload
	@echo "🤖 Starting investment-agent in development mode..."
	@uv run watchfiles "python -m investment_agent" investment_agent/
...
docker-build: ## Build Docker image
	...
	@docker build -f Dockerfile.agent -t investment_agent:latest .

.PHONY: docker-build-no-cache
docker-build-no-cache: ## Build Docker image without cache
	@echo "🐳 Building Docker image (no cache)..."
	@docker build --no-cache -f Dockerfile.agent -t investment_agent:latest .
...
docker-clean: docker-rm ## Clean up Docker images and containers
	...
	@docker rmi investment_agent:latest || true
...
docker-push: ## Push Docker image to Docker Hub
	...
	@docker tag investment_agent:latest para5/investment_agent:latest
	@docker push para5/investment_agent:latest
...
status: ## Show agent and Docker status
	...
	@docker ps -a | grep investment_agent || echo "No containers found"
	@echo "\nDocker Images:"
	@docker images | grep investment_agent || echo "No images found"
	@echo "\nAgent Health:"
